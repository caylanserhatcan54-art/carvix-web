...
export default function UploadPage() {
  ...
  async function submit() {
    if (!flowToken || !items.length) {
      alert("Sistem hazır değil veya dosya seçilmedi. Lütfen sayfayı yenileyin.");
      return;
    }
    if (items.some(it => !it.part)) {
      alert("Lütfen her fotoğraf için hangi parçaya ait olduğunu seçin.");
      return;
    }
    ...
    try {
      ...
      for (const partKey of Object.keys(grouped)) {
        ...
        const uploadRes = await fetch(`${API}/flows/${flowToken}/upload`, { 
          method: "POST", 
          body: form 
        });
      ...
      alert("Yükleme sırasında bir hata oluştu. Lütfen bağlantınızı kontrol edin.");
    } finally {
      setLoading(false);
    }
  }
        ...
        {!analysisReady ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <button onClick={submit} disabled={loading || items.length === 0} style={{ marginTop: '20px', padding: '18px', borderRadius: '16px', backgroundColor: '#fff', color: '#000', fontWeight: '900', border: 'none', cursor: 'pointer', opacity: (loading || items.length === 0) ? 0.5 : 1 }}>
              {loading ? "YÜKLENİYOR..." : "FOTOĞRAFLARI KAYDET VE DEVAM ET"}
            </button>
          </div>
        ) : (
